...
6. После отчета E2B-сессия остается активной 15 минут.
7. В чате каждый новый вопрос требует еще одного `run_python`, поэтому
   follow-up ответы также основаны на вычислениях, а не на пересказе отчета.
...
flowchart LR
    ...
    A -->|вопрос + sessionId| B
    C -->|run_python по тому же файлу| D
    C -->|submit_answer| B
...
- 15-минутная сессия датасета с продлением после сообщения;
...
| Диалог с датасетом | Сессия сохраняет E2B sandbox; каждый вопрос вызывает Python |
...
Команда запускает ESLint, TypeScript, 21 тест и production build.
...
- активные чат-сессии хранятся в памяти одного backend-процесса и
  завершаются при его перезапуске;
- E2B-песочница чата автоматически закрывается после 15 минут бездействия;
